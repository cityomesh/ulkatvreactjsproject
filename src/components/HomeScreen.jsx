...
//         fullScreen: { position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 9999, backgroundColor: '#000' }
//     };
...
const API_HOST = 'http://202.62.66.115:8080';
const CHANNELS_ENDPOINT = '/apiv2/channels/list';
const PALLYCON_TOKEN_URL = `${API_HOST}/apiv2/pallycon/TokenV2Issue`;
...
const HomeScreen = ({ accessToken }) => {
    ...
    const [playingUrl, setPlayingUrl] = useState('');
    const [drmToken, setDrmToken] = useState(''); // New state for DRM Token
    const [isFullScreen, setIsFullScreen] = useState(false);
    ...
    // --- Pallycon Token API Function ---
    const fetchPallyToken = async (contentId) => {
        let currentToken = accessToken || localStorage.getItem('ulka_token');
        
        if (!currentToken || currentToken === "DUMMY_TOKEN_OMI") {
            console.error("DRM Error: Valid Access Token ledu.");
            return null;
        }

        try {
            const authHeaderValue = currentToken.startsWith('auth=') 
                                    ? currentToken 
                                    : `auth=${currentToken}`;

            const response = await axios.get(PALLYCON_TOKEN_URL, {
                params: { 
            ...
            console.log("Pallycon API Response Status:", response.data.status_code);
            if (response.data.response_code === 200 && response.data.response_object.length > 0) {
                return response.data.response_object[0].base64Token;
            } else {
                console.error("Pallycon API Response Error Description:", response.data.error_description);
                return null;
            }
        } catch (err) {
            console.error("Pallycon Token Network Error:", err);
            return null;
        }
    };
    ...
    const handleKeyDown = useCallback(async (e) => { // Added async for API call
        if (isFullScreen) {
            if (e.key === 'Backspace' || e.key === 'Escape') {
                e.preventDefault();
                setIsFullScreen(false);
        ...
        if (activeSection === 'categories') {
            ...
            if (e.key === 'Enter') {
                ...
                if (selectedChannel) {
                    const cid = selectedChannel.content_id || selectedChannel.title;
                    
                    console.log("Requesting Token for:", cid);
                                        const token = await fetchPallyToken(cid);
                    ...
                    if (token) {
                        ...
                        alert(`Token failed for ${cid}. Please check if you are logged in with a real account.`);
                    }
                }
            }
        }
    ...
    }, [activeSection, focusedIdx, filteredChannels, allChannels, selectedCat, selectedLang, updateFilteredList, isFullScreen, playingUrl, accessToken]);
    ...
    if (isFullScreen && playingUrl) {
        return (
            <div style={styles.fullScreen}>
                <ShakaPlayer url={playingUrl} drmToken={drmToken} isEncrypted={true} />
            </div>
        );
    }
...
//         try {
//             const authHeader = currentToken.startsWith('auth=') ? currentToken : `auth=${currentToken}`;
...
//             if (response.data.response_code === 200 && response.data.response_object.length > 0) {
//                 //console.log(response.data.response_object[0].base64Token)
//                 return response.data.response_object[0].base64Token;
//             }
//             return null;
//         } catch (err) {
...
//         setIsLoading(true);
//         const cid = channel.content_id || channel.title;
//         const token = await fetchPallyToken(cid);
...
//         } else {
//             console.error("Could not get DRM Token");
//         }
//         setIsLoading(false);
//     };
...
//                 const response = await axios.post(`${API_HOST}${CHANNELS_ENDPOINT}`, { auth: currentToken });
//                 if (response.data.status_code === 200) {
//                     setAllChannels(response.data.response_object);
//                     updateFilteredList(response.data.response_object, 0, 0);
//                 }
//             } catch (err) { console.error("API Error:", err); }
//         };
//         fetchChannels();
...
//     const handleKeyDown = useCallback((e) => {
//         if (isFullScreen) {
//             if (e.key === 'Backspace' || e.key === 'Escape') { e.preventDefault(); setIsFullScreen(false); }
//             return;
//         }

//         if (e.key === 'Backspace' || e.key === 'Escape') { e.preventDefault(); window.location.href = '/exit'; return; }

//         if (activeSection === 'categories') {
//             if (e.key === 'ArrowDown') setFocusedIdx(p => Math.min(p + 1, CATEGORIES.length - 1));
//             if (e.key === 'ArrowUp') setFocusedIdx(p => Math.max(p - 1, 0));
//             if (e.key === 'ArrowRight') { setActiveSection('languages'); setFocusedIdx(selectedLang); }
//             if (e.key === 'Enter') { setSelectedCat(focusedIdx); updateFilteredList(allChannels, focusedIdx, selectedLang); }
//         } 
//         else if (activeSection === 'languages') {
//             if (e.key === 'ArrowDown') setFocusedIdx(p => Math.min(p + 1, LANGUAGES.length - 1));
//             if (e.key === 'ArrowUp') setFocusedIdx(p => Math.max(p - 1, 0));
//             if (e.key === 'ArrowLeft') { setActiveSection('categories'); setFocusedIdx(selectedCat); }
//             if (e.key === 'ArrowRight') { setActiveSection('channels'); setFocusedIdx(0); }
//             if (e.key === 'Enter') { setSelectedLang(focusedIdx); updateFilteredList(allChannels, selectedCat, focusedIdx); }
//         }
//         else if (activeSection === 'channels') {
//             if (e.key === 'ArrowDown') setFocusedIdx(p => Math.min(p + 1, filteredChannels.length - 1));
//             if (e.key === 'ArrowUp') setFocusedIdx(p => Math.max(p - 1, 0));
...
//             if (e.key === 'ArrowRight') { setActiveSection('player'); }
//             if (e.key === 'Enter') {
//                 const selected = filteredChannels[focusedIdx];
...
//         backgroundColor: '#FFFAE0', color: '#000', height: '60px',
//         display: 'flex', alignItems: 'center', justifyContent: 'center',
//         fontSize: '24px', fontWeight: '900', borderRadius: '4px'
//     };

//     const styles = {
//         container: { width: '100vw', height: '100vh', backgroundColor: '#000', color: '#fff', display: 'flex', padding: '25px', boxSizing: 'border-box', overflow: 'hidden', fontFamily: 'Arial, sans-serif', gap: '15px' },
//         column: { display: 'flex', flexDirection: 'column', backgroundColor: '#050505', gap: '10px', height: '100%' },
//         scrollArea: { flex: 1, overflowY: 'auto', padding: '5px' },
//         sidebarItem: (isFocused, isSelected) => ({
//             padding: '10px', marginBottom: '13px', borderRadius: '4px', textAlign: 'center',
//             border: isFocused ? '3px solid #f00' : '1px solid #222',
//             backgroundColor: isSelected ? '#4e4c4c' : 'transparent',
//             color: isFocused ? '#f00' : '#fff', fontSize: '20px', fontWeight: 'bold'
//         }),
...
//             border: isFocused ? '3px solid #f00' : 'none', transition: 'all 0.1s ease'
//         }),
//         playerWrapper: (isFocused) => ({
//             flex: 1, backgroundColor: '#111', borderRadius: '4px', 
//             border: isFocused ? '5px solid #00D1FF' : '2px solid #333',
...
//     if (isFullScreen && playingUrl) {
//         return <div style={styles.fullScreen}><ShakaPlayer url={playingUrl} drmToken={drmToken} /></div>;
//     }
...
//                             <div key={ch.id} style={styles.channelBox(activeSection === 'channels' && focusedIdx === i)}>
//                                 <img src={ch.icon_url} alt="" style={{ maxWidth: '90%', maxHeight: '90%' }} />
//                             </div>
//                         ))}
//                     </div>
//                 </div>
...
//                 </div>
//                 <div style={styles.playerWrapper(activeSection === 'player')}>
//                     {isLoading ? <div style={{color: '#00D1FF', fontSize: '24px'}}>Loading Stream...</div> :
//                      playingUrl ? <ShakaPlayer url={playingUrl} drmToken={drmToken} /> : 
//                      <div style={{textAlign: 'center'}}><img src={currentHover?.icon_url} style={{height: '200px'}} /><p>Press OK to Play</p></div>}
//                 </div>
//                 <div style={{ height: '100px', backgroundColor: '#00D1FF', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#000', fontSize: '30px', fontWeight: '900' }}>ADVERTISEMENT</div>
//             </div>
//         </div>
//     );
// };
